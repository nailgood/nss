...
namespace PayPalHandler
{
	...
	public class vtHttp
	{
		...
		string cUserAgent = "Portico HTTP .NET Client";
		...
		/// <summary>
		/// The psHttp Default Constructor
		/// </summary>
		public void psHTTP()
		{
		}
			...
			switch(this.nPostMode)
			{
				case HttpPostMode.UrlEncoded:
					this.oPostData.Write( Encoding.GetEncoding(1252).GetBytes(Key + "=" +
						vtHttpUtils.UrlEncode(Encoding.GetEncoding(1252).GetString(Value)) +
						"&") );
					break;
	...
	/// <summary>
	/// Enumeration of the various HTTP POST modes supported by psHTTP
	/// </summary>
