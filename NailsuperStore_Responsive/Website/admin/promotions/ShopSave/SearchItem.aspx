<%@ Page Language="VB" AutoEventWireup="false" CodeFile="SearchItem.aspx.vb" Inherits="admin_promotions_ShopSave_SearchItem" %>
...
<body onload="CheckTarget();">
                ...
                <CC:GridView ID="gvList" CellSpacing="2" CellPadding="2" runat="server" PageSize="20"
                    ...
                    <Columns>
                        <asp:TemplateField HeaderText="">
                            <ItemTemplate>
                                <%If Type = "1" Then%>
                                <asp:CheckBox ID="chk_SKU" runat="server" />
                                <%Else%>
                                <input type="radio" <%#getRadioSelect(Eval("SKU").toString()) %> onclick="CheckItemRadio('<%#Container.DataItem("SKU")%>','<%#Container.DataItem("IsActive") %>',this.checked);"
                                    name="SelectSigle" />
                                <%End If%>
                            </ItemTemplate>
                        </asp:TemplateField>
    ...
    <input type="hidden" runat="server" value="" id="hidSKUSelect" />
    <input type="hidden" runat="server" value="" id="hidIsActiveSelect" />
    <script type="text/javascript">
        function CheckItem(id, status, isactive) {
            var idSelect = document.getElementById('<%=hidSKUSelect.ClientID %>').value;
            if (status) {
                idSelect += id + ';';
            }
            else idSelect = idSelect.replace(id + ';', '');
            document.getElementById('<%=hidSKUSelect.ClientID %>').value = idSelect;
            document.getElementById('<%=hidIsActiveSelect.ClientID %>').value = isactive;
        }
        function CheckItemRadio(id,isactive, status) {

            document.getElementById('<%=hidSKUSelect.ClientID %>').value = id;
            document.getElementById('<%=hidIsActiveSelect.ClientID %>').value = isactive;
        }
        ...
        function Save() {
            ...
            var id = document.getElementById('<%=hidSKUSelect.ClientID %>').value;
            var isactive = document.getElementById('<%=hidIsActiveSelect.ClientID %>').value;
            if (id != '') {
                SetParentData('1', id,isactive);
                //window.opener.SetValue('1',id)           
                window.close();
            }
        }
        function SetParentData(save, data,isactive) {
            var brow = GetBrowser();
            if (brow == 'ie') {
                window.returnValue = data;
            }
            else {
                window.opener.SetValue('1', data, isactive);
            }
        }
