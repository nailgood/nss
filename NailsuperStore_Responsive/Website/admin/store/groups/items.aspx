<%@ Page Language="VB" AutoEventWireup="false" MasterPageFile="~/includes/masterpage/admin.master" Title="" CodeFile="items.aspx.vb" Inherits="admin_store_groups_related"  %>
<asp:content ContentPlaceHolderId="ph" ID="Content" runat="server">
	...
	function InitializeQuery() {
		InitQueryCode('LookupField', '/admin/ajax.aspx?f=DisplayItemsEx&ItemGroupId=<%=ItemGroupId%>&q=', MyCallback);
	}
	...
	function GetItemInfo() {
		...
		xml = getXMLHTTP();
		if(xml){
			xml.open("GET","/admin/Ajax.aspx?f=GetItemInfoEx&ItemGroupId=<%=ItemGroupId%>&ItemId=" + getValue(document.getElementById('ItemId')),true);
			...
			<TBODY>
				<tr>
					...
					<td class="field" width="400">
						...
						<p>
							<asp:button id="btnAdd" runat="server" Text="Add Coordinating Item" cssClass="btn" Enabled="False"></asp:button></p>
					</td>
					<td valign="top">
					</td>
				</tr>
				...
				<td width="435">
					<asp:DataGrid BorderWidth="0" PageSize="20" AllowPaging="True" AllowSorting="True" CellPadding="2"
						...
						<Columns>
							<asp:TemplateColumn>
								<ItemTemplate>
									<CC:ConfirmLink enableviewstate=False Message="Are you sure that you want to remove this item?" runat="server" NavigateUrl='<%# "remove.aspx?ItemGroupId=" & DataBinder.Eval(Container.DataItem, "ItemGroupId") & "&ItemId=" & DataBinder.Eval(Container.DataItem, "ItemId") & "&" & params %>' ImageUrl="/includes/theme-admin/images/delete.gif" ID="Confirmlink1"/>
								</ItemTemplate>
							</asp:TemplateColumn>
							...
							<asp:TemplateColumn SortExpression="Y">
								...
								<ItemTemplate>
									<asp:Repeater runat="server" ID="rpt">
										...
										<ItemTemplate><%#Container.DataItem("OptionName") & ": <strong>" & Container.DataItem("ChoiceName") & "</strong>"%></ItemTemplate>
									</asp:Repeater>
								</ItemTemplate>
							</asp:TemplateColumn>
							<asp:TemplateColumn SortExpression="Y">
