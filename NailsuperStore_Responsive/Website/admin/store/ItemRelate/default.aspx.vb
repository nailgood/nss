...
Partial Class admin_store_itemrelate_Index
    Inherits AdminPage
    ...
    Private Sub BindList()
        ...
        SQLFields = "SELECT TOP " & (gvList.PageIndex + 1) * gvList.PageSize & " si.ItemId, si.SKU, si.ItemName, si.IsActive, sb.BrandName, g.GroupName, "
        SQL = " FROM storeitemrelate sr, StoreItem si left outer join storeitemgroup g on si.itemgroupid = g.itemgroupid left outer join storebrand sb on si.brandid = sb.brandid where sr.itemidrelate = si.itemid and sr.isactive = 1"
        ...
        If Not F_ItemType.Text = String.Empty Then
            If F_ItemType.Text = "0" Then SQL = SQL & Conn & "itemgroupid is null " Else SQL = SQL & Conn & "itemgroupid is not null "
            Conn = " AND "
        End If
