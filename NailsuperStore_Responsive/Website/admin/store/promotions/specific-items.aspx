<%@ Page Language="VB" AutoEventWireup="false" MasterPageFile="~/includes/masterpage/admin.master" Title="Store Promotion" CodeFile="specific-items.aspx.vb" Inherits="SpecificItems"  %>
...
function InitializeQuery() {
	InitQueryCode('LookupField', '/admin/ajax.aspx?f=DisplayItemsAndDepartments&q=', MyCallback);
}
...
function GetItemInfo() {
	...
	if(xml){
		...
		if (temp[1] == 'Item') {
			...
			xml.open("GET","/admin/ajax.aspx?f=GetDepartmentInfo&DepartmentId=" + temp[0],true);
			xml.onreadystatechange = function() {
				if(xml.readyState==4 && xml.responseText) {
					if (xml.responseText.length > 0) {
						aData = xml.responseText.split('|');
